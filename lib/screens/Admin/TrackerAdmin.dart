...
class TrackerAdmin extends StatefulWidget {
  final String service_id;
  const TrackerAdmin({Key? key, required this.service_id}) : super(key: key);

  @override
  State<TrackerAdmin> createState() => _MyStatefulWidgetState(this.service_id);
}

class _MyStatefulWidgetState extends State<TrackerAdmin> {
  int _index = 0;
  String _issue = '';
  final String service_id;
  ...
  void updateStatus(new_index) async{
    ...
    final response = await http
        .get(Uri.parse(globals.baseURL+'/updateService/'+new_index.toString()+'/'+service_id));
    if (response.statusCode == 200) {
      var vals = json.decode(response.body);
      print(vals);
      Navigator.push(
